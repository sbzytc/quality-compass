...
-- Recreate clone function in private schema
CREATE OR REPLACE FUNCTION private.clone_company_as_sandbox(_source_company_id uuid)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION private.clone_company_as_sandbox(uuid) FROM PUBLIC;

-- Recreate the auto-trigger to call the private function
...
BEGIN
  ...
  PERFORM private.clone_company_as_sandbox(NEW.id);
  RETURN NEW;
END; $$;

-- Drop the public-schema copies so they no longer sit on the API surface
DROP FUNCTION IF EXISTS public.clone_company_as_sandbox(uuid);

-- The public.trigger_auto_create_sandbox function is a trigger function; explicitly revoke
-- direct execute so it can't be invoked via RPC by clients.
REVOKE EXECUTE ON FUNCTION public.trigger_auto_create_sandbox() FROM PUBLIC, anon, authenticated;

-- Backfill existing real companies now that the schema is stable
DO $$
DECLARE r record;
BEGIN
  FOR r IN
    ...
    WHERE c.is_sandbox = false
      ...
      AND NOT EXISTS (SELECT 1 FROM public.companies s WHERE s.sandbox_of_company_id = c.id)
  LOOP
    PERFORM private.clone_company_as_sandbox(r.id);
  END LOOP;
END $$;
