...
async function executeTool(
  ...
  userId: string,
  scope: { branchId: string | null; hasFullAccess: boolean }
): Promise<string> {
  try {
    // Server-side authorization: for non-admin/super_admin users we FORCE
    // branch_id to the caller's own branch on every tool call, regardless of
    // whatever the model tries to pass. This is defense-in-depth on top of
    // RLS — the model can no longer probe other branches by prompt injection.
    if (!scope.hasFullAccess) {
      const branchScopedTools = new Set([
        "get_evaluations",
        "get_findings",
        "get_tasks",
        "create_task",
      ...
      if (branchScopedTools.has(toolName)) {
        if (!scope.branchId) {
          return JSON.stringify({
            error: "forbidden",
            message: "لا يوجد فرع مرتبط بحسابك، لا يمكن تنفيذ هذا الطلب.",
          });
        }
        if (args.branch_id && args.branch_id !== scope.branchId) {
          return JSON.stringify({
            error: "forbidden",
            message: "صلاحياتك تسمح فقط بالوصول لبيانات فرعك.",
          });
        }
        args.branch_id = scope.branchId;
      }
    }
    switch (toolName) {
...
serve(async (req) => {
  ...
  try {
    ...
    const scope = { branchId: userBranchId, hasFullAccess };
